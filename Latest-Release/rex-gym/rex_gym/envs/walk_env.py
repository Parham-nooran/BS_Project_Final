...
class RexWalkEnv(rex_gym_env.RexGymEnv):
  ...
  """
    ...
    def setup_ui(self, base_x, step, period):
        self.base_x_ui = self._ranges["base_x"][1] # self._pybullet_client.addUserDebugParameter("base_x", self._ranges["base_x"][0], self._ranges["base_x"][1], base_x)
        self.base_y_ui = self._ranges["base_y"][2] # self._pybullet_client.addUserDebugParameter("base_y", self._ranges["base_y"][0], self._ranges["base_y"][1], self._ranges["base_y"][2])
        self.base_z_ui = self._ranges["base_z"][2] # self._pybullet_client.addUserDebugParameter("base_z", self._ranges["base_z"][0], self._ranges["base_z"][1], self._ranges["base_z"][2])
        self.roll_ui = self._ranges["roll"][2] # self._pybullet_client.addUserDebugParameter("roll", self._ranges["roll"][0], self._ranges["roll"][1], self._ranges["roll"][2])
        self.pitch_ui = self._ranges["pitch"][2] # self._pybullet_client.addUserDebugParameter("pitch", self._ranges["pitch"][0], self._ranges["pitch"][1], self._ranges["pitch"][2])
        self.yaw_ui = self._ranges["yaw"][2] #self._pybullet_client.addUserDebugParameter("yaw", self._ranges["yaw"][0], self._ranges["yaw"][1], self._ranges["yaw"][2])
        self.step_length_ui = 0.7 # self._pybullet_client.addUserDebugParameter("step_length", -0.7, 0.7, step)
        self.step_rotation_ui = 1.5 # self._pybullet_client.addUserDebugParameter("step_rotation", -1.5, 1.5, 0.)
        self.step_angle_ui = 180 # self._pybullet_client.addUserDebugParameter("step_angle", -180., 180., 0.)
        self.step_period_ui = 0.9 # self._pybullet_client.addUserDebugParameter("step_period", 0.2, 0.9, period)
