...
FLAG_TO_FILENAME = {
    ...
    'maze': "parham-envs/parham_envs/resources/statics/maze.png"
}
...
class Terrain:
    def __init__(self, terrain_source, terrain_id, columns=256, rows=256, width=0.5, length=0.5, height=1):
        random.seed(10)
    ...
    def generate_terrain(self, client, height_perturbation_range=0.05):
        ...
        if self.terrain_source == 'random':
            ...
            terrain_shape = client.createCollisionShape(
                ...
                meshScale=[self.width, self.length, self._height],
        ...
        if self.terrain_source == 'csv':
            ...
            client.resetBasePositionAndOrientation(terrain, [38, 27, 2], [0, 0, 0, 1])
        ...
        if self.terrain_source == 'png':
            ...
            if self.terrain_id == "mounts":
                ...
                client.resetBasePositionAndOrientation(terrain, [-10, -10, 0], [0, 0, 0, 1])
            else:
                terrain_shape = client.createCollisionShape(
                shapeType=client.GEOM_HEIGHTFIELD,
                ...
                x, y, _ = img.shape
                client.resetBasePositionAndOrientation(terrain, [-x/2+60, -y/2+60, 1], [0, 0, 0, 1])
